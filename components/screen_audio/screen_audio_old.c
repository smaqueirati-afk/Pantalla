/**
 * @file screen_audio.c � Audio Console integrada al proyecto ESP32-P4
 */
...
/* -- Estilos ----------------------------------------------- */
static lv_style_t sty_screen, sty_topbar;
static lv_style_t sty_nav_btn, sty_nav_btn_pr;
static lv_style_t sty_mute_btn, sty_mute_btn_active, sty_mute_btn_pr;
static lv_style_t sty_divider, sty_surface_box;
static lv_style_t sty_slider_bg, sty_slider_ind, sty_knob;
static lv_style_t sty_ch_lbl, sty_value_lbl, sty_scale_lbl, sty_section_lbl;
static lv_style_t sty_bar_green, sty_bar_yellow, sty_bar_orange;
static lv_style_t sty_vu_bg, sty_vu_fill;

/* -- Helpers ---------------------------------------------- */
static inline int32_t db_to_pct(int16_t db) { return (int32_t)(db + 12) * 100 / 24; }
static inline int16_t pct_to_db(int32_t pct) { return (int16_t)(pct * 24 / 100 - 12); }
...
static void init_styles(void) {
    ...
    lv_style_set_text_color(&sty_mute_btn_active, lv_color_white());
    ...
    lv_style_set_shadow_color(&sty_knob, lv_color_make(0,0,0));
...
static void cb_slider_press(lv_event_t *e) {
    ...
    lv_coord_t abs_y = lv_obj_get_y(bar) + lv_obj_get_y(lv_obj_get_parent(bar)) + TOPBAR_H + 1;
    ...
    if (ch == &ac.master) {
        char path[32];
        snprintf(path, sizeof(path), "/api/volume?vol=%d", (int)((ch->value_db + 12) * 100 / 24));
        volumio_get_path(path);
        ha_media_set_volume("media_player.spotify_premiun", (ch->value_db + 12) / 24.0f);
    } else if (ch == &ac.bass) {
...
/* -- Build EQ channel ------------------------------------- */
static void build_eq_channel(lv_obj_t *parent, eq_ch_t *ch,
                               const char *name, int16_t default_db,
                               lv_coord_t col_x, lv_coord_t col_y) {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        lv_obj_set_pos(m, origin, slider_y + (lv_coord_t)(i * SLIDER_H / 6) - 5);
    }
    ...
    lv_obj_set_pos(ch->knob, track_x + 2,
                   slider_y + SLIDER_H - (lv_coord_t)(pct * SLIDER_H / 100) - KNOB_H / 2);
...
/* -- Build spectrum --------------------------------------- */
static void build_spectrum(lv_obj_t *parent, lv_coord_t x, lv_coord_t y,
    ...
    for (int i = 0; i < SPECTRUM_BARS; i++) {
        ...
        lv_obj_set_pos(ac.spec_peaks[i], bx, bars_y + bars_h - bar_h_px - 4);
        ...
        if      (i >= 20) lv_obj_add_style(ac.spec_bars[i], &sty_bar_orange, 0);
        else if (i >= 16) lv_obj_add_style(ac.spec_bars[i], &sty_bar_yellow, 0);
...
/* -- Build VU meters -------------------------------------- */
static void build_vu_meters(lv_obj_t *parent, lv_coord_t x, lv_coord_t y, lv_coord_t w) {
    lv_obj_t *box = lv_obj_create(parent);
...
static void cb_mute(lv_event_t *e) {
    ...
    /* Aplicar mute */
    float vol = ac.muted ? 0.0f : (float)(ac.master.value_db + 12) / 24.0f;
    ha_media_set_volume("media_player.spotify_premiun", vol);
    char path[32];
    snprintf(path, sizeof(path), "/api/volume?vol=%d", ac.muted ? 0 : (int)((ac.master.value_db + 12) * 100 / 24));
    volumio_get_path(path);
}
...
static void build_left_panel(lv_obj_t *parent) {
    ...
    build_eq_channel(panel, &ac.master, "MASTER", 0, x_master, 12);
    build_eq_channel(panel, &ac.bass,   "BASS",   0, x_bass,   12);
    build_eq_channel(panel, &ac.treble, "TREBLE", 0, x_treble, 12);
...
static void build_right_panel(lv_obj_t *parent) {
    ...
    build_spectrum(parent, x0 + pad, y0 + pad, RIGHT_W - pad * 2, spec_h);
    build_vu_meters(parent, x0 + pad, y0 + pad + spec_h + pad, RIGHT_W - pad * 2);
}

/* -- EQ timer animacion ----------------------------------- */
static int s_eq_h[SPECTRUM_BARS] = {0};
static int s_peak_h[SPECTRUM_BARS] = {0};
static int s_peak_hold[SPECTRUM_BARS] = {0};
static int s_volume_pct = 70;

static void eq_anim_timer(lv_timer_t *t) {
    (void)t;
    int maxh = 150;
    for (int i = 0; i < SPECTRUM_BARS; i++) {
        float ff = 0.5f + 0.8f * sinf((float)i / SPECTRUM_BARS * 3.14159f);
        int target = (int)(10 + ff * (rand() % 120) * s_volume_pct / 100.0f);
        if (target > maxh) target = maxh;
        s_eq_h[i] = (s_eq_h[i] * 4 + target) / 5;
        if (s_eq_h[i] > s_peak_h[i]) {
            s_peak_h[i] = s_eq_h[i];
            s_peak_hold[i] = 15;
        } else {
            if (s_peak_hold[i] > 0) s_peak_hold[i]--;
            else if (s_peak_h[i] > 0) s_peak_h[i] -= 2;
        }
        if (!ac.spec_bars[i] || !ac.spec_peaks[i]) continue;
        bsp_display_lock(0);
        lv_obj_set_height(ac.spec_bars[i], LV_MAX(3, s_eq_h[i]));
        lv_obj_set_y(ac.spec_peaks[i], lv_obj_get_y(ac.spec_bars[i]) - 4);
        bsp_display_unlock();
    }
    /* VU meters */
    if (!ac.vu_l || !ac.vu_r) return;
    lv_coord_t tw = lv_obj_get_width(lv_obj_get_parent(ac.vu_l));
    int vl = (int)(10 + (rand() % 80) * s_volume_pct / 100.0f);
    int vr = (int)(10 + (rand() % 80) * s_volume_pct / 100.0f);
    if (vl > tw) vl = tw;
    if (vr > tw) vr = tw;
    bsp_display_lock(0);
    lv_obj_set_width(ac.vu_l, vl);
    lv_obj_set_width(ac.vu_r, vr);
    lv_obj_set_x(ac.vu_peak_l, LV_MAX(0, vl - 3));
    lv_obj_set_x(ac.vu_peak_r, LV_MAX(0, vr - 3));
    bsp_display_unlock();
}
...
lv_obj_t *screen_audio_create(lv_event_cb_t back_cb) {
    ...
    ac.back_cb = back_cb;
    for (int i = 0; i < SPECTRUM_BARS; i++) s_eq_h[i] = 20 + rand() % 80;

    s_screen = lv_obj_create(NULL);
    init_styles();
    lv_obj_add_style(s_screen, &sty_screen, 0);
    build_topbar(s_screen);
    build_left_panel(s_screen);
    build_right_panel(s_screen);

    s_eq_timer = lv_timer_create(eq_anim_timer, 100, NULL);
    return s_screen;
}
...
void screen_audio_set_volume(int vol) {
    s_volume_pct = vol;
    ac.master.value_db = (int16_t)(vol * 24 / 100 - 12);
    update_eq_visual(&ac.master);
}

void screen_audio_update_eq(void) { }
void screen_audio_set_back_cb(lv_event_cb_t cb) { ac.back_cb = cb; }
